 ...
 */
public class Main {
    private static CodeWriter cw = new CodeWriter();
    /**
    ...
    private static void runTranslator(String fileName, String filePath) {
        ...
        while (cp.hasMoreCommands()) {
            ...
            switch (commandType) {
                ...
                case C_LABEL:
                    cw.writeLabel(cp.getArg1()/*, false*/);
                    break;
                case C_GOTO:
                    cw.writeGoto(cp.getArg1()/*, false*/);
                    break;
                case C_IF:
                    cw.writeIf(cp.getArg1());
                    break;
                case C_FUNCTION:
                    cw.writeFunction(cp.getArg1(), Integer.parseInt(cp.getArg2()));
                    break;
                case C_RETURN:
                    cw.writeReturn();
                    break;
                case C_CALL:
                    cw.writeCall(cp.getArg1(), Integer.parseInt(cp.getArg2()));
                    break;
            }
            cp.advance();
        }
